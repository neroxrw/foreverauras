...
local function getAuraMatchesList(name, showSpellIdRecommendation)
  ...
  if ids then
    ...
    if showSpellIdRecommendation then
      local tip = L["ForeverAuras recommends using spell ids instead of names. Spell ids are automatically localized."]
    ...
    if isExactSpellId then
      ...
      aura_options[iconOption].image = function()
        ...
        elseif input and input ~= "" then
          icon = "Interface\\AddOns\\ForeverAuras\\Media\\Textures\\info"
        end
        return icon and tostring(icon) or "", 18, 18
      end
...
local function GetBuffTriggerOptions(data, triggernum)
  ...
  local aura_options = {
    ...
    useArenaSpec = {
      ...
      order = 67.8,
      hidden = function() return
        not (trigger.type == "aura2" and trigger.unit == "arena")
      end
    },
    arena_spec = {
      type = "multiselect",
      ...
      values = OptionsPrivate.Private.spec_types_all,
      hidden = function()
        return not (trigger.type == "aura2" and trigger.unit == "arena" and trigger.useArenaSpec)
      end,
      order = 67.9
    },
    arena_specSpace = {
    ...
    --   order = 69.81,
    --   width = ForeverAuras.doubleWidth,
    --   hidden = function() return not (trigger.type == "aura2" and (trigger.unit == "group" or trigger.unit == "raid" or trigger.unit == "party") and true) end
    -- },
    ignoreInvisible = {
      type = "toggle",
