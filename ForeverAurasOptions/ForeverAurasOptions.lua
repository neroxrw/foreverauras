...
local function EnsureDisplayButton(data)
  ...
  if not(displayButtons[id]) then
    ...
    if(displayButtons[id]) then
      ...
      print("|cFF8800FFForeverAuras|r: Error creating button for", id);
    end
  end
end
...
local function LayoutDisplayButtons(msg)
  ...
  local func1 = function()
   ...
   OptionsPrivate.Private:Async({name = "LayoutDisplayButtons2"}, func2)
  end

  OptionsPrivate.Private:Async({name = "LayoutDisplayButtons1"}, func1)
end
...
function OptionsPrivate.SortDisplayButtons(filter, overrideReset, id)
  ...
  if OptionsPrivate.Private.CompanionData.slugs then
    ...
    for id, aura in pairs(ForeverAurasSaved.displays) do
...
function OptionsPrivate.DropIndicator()
  ...
  if not indicator then
    ...
    groupTexture:SetTexture("Interface\\AddOns\\ForeverAuras\\Media\\Textures\\Square_FullWhite")
...
function OptionsPrivate.OpenModelPicker(baseObject, path)
  if not(C_AddOns.IsAddOnLoaded("ForeverAurasModelPaths")) then
...
function OptionsPrivate.OpenTriggerTemplate(data, targetId)
  if not(C_AddOns.IsAddOnLoaded("ForeverAurasTemplates")) then
...
function OptionsPrivate.UpdateTextReplacements(frame, data)
  ...
  -- Create a modified ForeverAurasSnippetButton for each property and add it to ScrollList
