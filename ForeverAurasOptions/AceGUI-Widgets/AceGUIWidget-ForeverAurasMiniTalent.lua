...
local methods = {
  ...
  SetList = function(self, list)
    ...
    for _, button in ipairs(self.buttons) do
      minX = minX and math.min(minX, button.posX) or button.posX
      minY = minY and math.min(minY, button.posY) or button.posY
      maxX = maxX and math.max(maxX, button.posX) or button.posX
      maxY = maxY and math.max(maxY, button.posY) or button.posY
    end
    local padding = buttonSizePadded
    local width = minX and maxX - minX + 2 * padding or 440
    local height = minY and maxY - minY + 2 * padding or 100
    for _, button in ipairs(self.buttons) do
      button.posX = button.posX - minX + padding
      button.posY = button.posY - minY + padding
    end
    self.scale = self.saveSize.fullWidth / width
    self.saveSize.fullHeight = height * self.scale
    self.background:Hide()
