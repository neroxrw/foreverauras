...
-- in 12.1 there is no way to iterate over all auras on the unit while auras are secret
...
local aurasAreLocked = C_Secrets and type(C_Secrets.ShouldAurasBeSecret) == "function"
    ...
    if not C_Secrets.ShouldAurasBeSecret() then
      ...
      if sfs then
        for spellID in next, sfs do
          ...
          if auraData and not issecretvalue(auraData) then
...
Private.frames["ForeverAuras Buff2 Frame"] = Buff2Frame
...
local function EventHandler(frame, event, arg1, arg2, ...)
  ...
  elseif event == "UNIT_AURA" then
    ...
    if newAPI then
      ...
      if arg2 == nil or issecretvalue(arg2) or issecretvalue(arg2.isFullUpdate) or arg2.isFullUpdate or issecretvalue(arg2.addedAuras) or issecretvalue(arg2.removedAuraInstanceIDs) or issecretvalue(arg2.updatedAuraInstanceIDs) then
        ScanUnit(time, arg1)
      else
...
function BuffTrigger.Add(data)
  ...
  for triggernum, triggerData in ipairs(data.triggers) do
    ...
    if trigger.type == "aura2" then
      ...
      local effectiveArenaSpec = (trigger.unit == "arena" and trigger.useArenaSpec and trigger.arena_spec) or nil
