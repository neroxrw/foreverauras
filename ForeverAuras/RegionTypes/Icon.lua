...
local function create(parent, data)
  ...
  --The reason is so that ForeverAuras cooldown frames can interact properly with OmniCC
  ...
  then
    ElvUI[1]:RegisterCooldown(cooldown, "ForeverAuras");
  end
...
local function modify(parent, region, data)
  ...
  if MSQ then
    ...
    if region.masqueId ~= masqueId then
      ...
      region.MSQGroup = MSQ:Group("ForeverAuras", region.masqueId, data.uid);
