...
-- An alias for ForeverAurasSaved, the SavedVariables
-- Noteable properties:
--  debug: If set to true, WeakAura.debug() outputs messages to the chat frame
--  displays: All aura settings, keyed on their id

---@class ForeverAurasSaved
local db;

-- While true no events are handled. E.g. ForeverAuras is paused while the Options dialog is open
local paused = true;
...
local function CreateTalentCache()
  -- Forever does not use the legacy fixed-row retail talent cache.
  -- Modern trait handling is separate; keep this lookup empty for old imports.
end

local function CreatePvPTalentCache()
  -- PvP talent slots are not available on Forever.
  -- The empty cache above keeps imported settings and options well formed.
end
...
do -- Archive stuff
  ...
  local function OpenArchive()
    if Archivist:IsInitialized() then
      ...
      if not C_AddOns.IsAddOnLoaded("ForeverAurasArchive") then
        local ok, reason = C_AddOns.LoadAddOn("ForeverAurasArchive")
        if not ok then
          reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
          error(string.format(L["Could not load ForeverAuras Archive, the addon is %s"], reason))
        end
      end
      if type(ForeverAurasArchive) ~= "table" then
        ForeverAurasArchive = {}
      end
      Archivist:Initialize(ForeverAurasArchive)
    end
    return Archivist
  end
...
function Private.Login(takeNewSnapshots)
  local loginFunc = (function() -- coroutine.create
    ...
    if db.history then
      ...
      -- history is now in archive so we can shrink ForeverAurasSaved
      db.history = nil
    end
    ...
    for id, data in pairs(db.displays) do
      if(id ~= data.id) then
        print("|cFF8800FFForeverAuras|r detected a corrupt entry in ForeverAuras saved displays - '"..tostring(id).."' vs '"..tostring(data.id).."'" );
        data.id = id;
      end
...
loadedFrame:SetScript("OnEvent", function(self, event, ...)
  if(event == "ADDON_LOADED") then
    if(... == ADDON_NAME) then
      ...
      -- if not db.disableRuLocale and ForeverAuras.LoadLocale_ruRU then
      --   ForeverAuras.LoadLocale_ruRU()
      -- end
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  local player, realm, zone = Private.ExecEnv.UnitName("player"), GetRealmName(), GetRealZoneText()
  ...
  if takeSnapshots then
    ...
    if #order > 0 then
      Private:Async({name = "snapshot"}, function()
        prettyPrint(L["ForeverAuras is creating a rollback snapshot of your auras. This snapshot will allow you to revert to the current state of your auras if something goes wrong. This process may cause your framerate to drop until it is complete."])
...
do
  ...
  LGF.RegisterCallback("ForeverAuras", "FRAME_UNIT_UPDATE", frame_monitor_callback)
  LGF.RegisterCallback("ForeverAuras", "FRAME_UNIT_ADDED", frame_monitor_callback)
  LGF.RegisterCallback("ForeverAuras", "FRAME_UNIT_REMOVED", frame_monitor_callback)
end
...
local function ensureMouseFrame()
  ...
  label:SetText("ForeverAuras Anchor");
...
function Private.ensurePRDFrame()
  ...
  texture:SetAllPoints(moverFrame);
  texture:SetTexture("Interface\\AddOns\\ForeverAuras\\Media\\Textures\\PRDFrame");
...
function Private.ExecEnv.ParseBossModCheck(input)
  ...
  -- check if it is enabled with ForeverAuras.IsBossModEnabled(encounterId)
